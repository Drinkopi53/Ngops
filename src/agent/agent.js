...
export class Agent {
    ...
    async _setupEventHandlers(save_data, init_message) {
        ...
        if (save_data?.last_sender) {
            ...
            if (!settings.manual_only)
                ...
                this.openChat(`Manual-only mode active. I am ${this.name}. Send me commands like !stop, !goToPlayer, etc.`);
        }
        else {
    ...
    async handleMessage(source, message, max_responses=null) {
        ...
        if (!self_prompt && !from_other_bot) { // from user, check for forced commands
            ...
            // If manual_only mode is active and message contains no command, ignore it
            if (settings.manual_only) {
                this.routeResponse(source, `[Manual-only mode] No command detected. Use commands like !stop, !goToPlayer, etc.`);
                return false;
            }
        }
